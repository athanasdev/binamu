...
@section('title')
<title>Set Login Password | {{ config('app.name') }} </title>
    ...
    <main>
        <section class="section-padding mt-5">
            <div class="max-w-[600px] mx-auto px-4">
                <h2 class="text-2xl font-semibold text-center mb-5">Update Your Login Password</h2>
                    @include('frontend.layouts.flash-message')
                <form action="{{ route('update_login_password') }}" method="post" enctype="multipart/form-data" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    @csrf
